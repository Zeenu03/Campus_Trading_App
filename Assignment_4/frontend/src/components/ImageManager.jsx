...
/**
 * ImageManager: add/remove images for a listing (owner only).
 * @param {Object} props
 * @param {Object} props.listing - listing with images array
 * @param {Function} props.onUpdate - (updatedListing) => void, called after add/remove
 * @param {string} props.className - optional CSS class
 */
export default function ImageManager({ listing, onUpdate, className = '' }) {
  ...
  const handleAddClick = () => {
    if (images.length >= MAX_IMAGES) {
      toast.error(`Maximum ${MAX_IMAGES} images per listing`);
      return;
    }
  ...
  const handleFileChange = async (e) => {
    ...
    e.target.value = '';
    if (!file || !listing?.listing_id) return;
    ...
    setUploading(true);
    try {
      const img = await api.postImage(`/listings/${listing.listing_id}/images`, file);
      onUpdate?.({ ...listing, images: [...images, img] });
  ...
  const handleRemove = async (img) => {
    if (!listing?.listing_id || removing) return;
    if (!confirm('Remove this image?')) return;
    setRemoving(img.image_id);
    try {
      await api.delete(`/listings/${listing.listing_id}/images/${img.image_id}`);
      onUpdate?.({
        ...listing,
