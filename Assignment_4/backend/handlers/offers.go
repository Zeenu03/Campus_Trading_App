...
func loadOfferByID(ctx context.Context, offerID int) (*sql.DB, *offerLookup, error) {
	for _, shardDB := range appdb.AllShardConnections() {
		var offer offerLookup
		err := shardDB.QueryRowContext(ctx,
			`SELECT o.ListingID, o.BuyerID, l.SellerID, o.OfferedPrice, o.SellerAskingPrice,
			        o.AgreedPrice, o.OfferStatus, o.Reason, o.SubmittedDate, o.ResponseDate
		   FROM Offer o
		   JOIN Listing l ON l.ListingID = o.ListingID
		  WHERE o.OfferID = ?`, offerID,
		).Scan(&offer.ListingID, &offer.BuyerID, &offer.SellerID, &offer.OfferedPrice,
			&offer.SellerAskingPrice, &offer.AgreedPrice, &offer.OfferStatus,
			&offer.Reason, &offer.SubmittedDate, &offer.ResponseDate)
		if err == nil {
			return shardDB, &offer, nil
		}
...
func GetMyOffer(w http.ResponseWriter, r *http.Request) {
	...
	err = listingDB.QueryRowContext(ctx,
		`SELECT o.OfferID, o.ListingID, o.BuyerID, o.OfferedPrice, o.SellerAskingPrice,
             o.AgreedPrice, o.OfferStatus, o.Reason, o.SubmittedDate, o.ResponseDate
          FROM Offer o
          WHERE o.ListingID = ? AND o.BuyerID = ?`, listingID, memberID,
	).Scan(&o.OfferID, &o.ListingID, &o.BuyerID, &o.OfferedPrice,
		&o.SellerAskingPrice, &o.AgreedPrice, &o.OfferStatus, &o.Reason,
	...
	if o.BuyerName == "" {
		o.BuyerName = memberNameByID(ctx, o.BuyerID)
	}
...
func ListOffersForListing(w http.ResponseWriter, r *http.Request) {
	...
	err = listingDB.QueryRowContext(ctx,
		`SELECT SellerID FROM Listing WHERE ListingID = ?`, listingID).Scan(&sellerUserID)
	if err == sql.ErrNoRows {
	...
	rows, err := listingDB.QueryContext(ctx,
		`SELECT o.OfferID, o.ListingID, o.BuyerID, o.OfferedPrice, o.SellerAskingPrice,
             o.AgreedPrice, o.OfferStatus, o.Reason, o.SubmittedDate, o.ResponseDate
          FROM Offer o
	...
	for rows.Next() {
		var o models.Offer
		_ = rows.Scan(&o.OfferID, &o.ListingID, &o.BuyerID,
...
func CreateOffer(w http.ResponseWriter, r *http.Request) {
	...
	offerID, err := nextRecordID(ctx, "Offer", "OfferID")
	if err != nil {
		respondError(w, http.StatusInternalServerError, "failed to allocate offer id")
		return
	}
	threadID, err := nextRecordID(ctx, "MessageThread", "ThreadID")
	if err != nil {
		respondError(w, http.StatusInternalServerError, "failed to allocate thread id")
		return
	}
	watchlistID, err := nextRecordID(ctx, "Watchlist", "WatchlistID")
	if err != nil {
		respondError(w, http.StatusInternalServerError, "failed to allocate watchlist id")
		return
	}
	...
	_, err = tx.ExecContext(ctx,
		`INSERT INTO MessageThread (ThreadID, ListingID, BuyerID, OfferID)
         VALUES (?, ?, ?)
         ON DUPLICATE KEY UPDATE OfferID = VALUES(OfferID)`,
		threadID, listingID, memberID, offerID,
	)
	if err != nil {
	...
	_, _ = tx.ExecContext(ctx,
		`INSERT IGNORE INTO Watchlist (WatchlistID, MemberID, ListingID) VALUES (?, ?, ?)`,
		watchlistID, memberID, listingID)
...
func AcceptOffer(w http.ResponseWriter, r *http.Request) {
	...
	ctx := r.Context()
	offerDB, offer, err := loadOfferByID(ctx, offerID)
	if err == sql.ErrNoRows {
	...
	tx, err := offerDB.BeginTx(ctx, nil)
	if err != nil {
		respondError(w, http.StatusInternalServerError, "tx failed")
		return
	}
	...
	err = tx.QueryRowContext(ctx,
		`SELECT Status FROM Listing WHERE ListingID = ? FOR UPDATE`, offer.ListingID,
	...
	_, _ = tx.ExecContext(ctx,
		`UPDATE Listing SET Status = 'Sold', LastModifiedDate = NOW() WHERE ListingID = ?`, offer.ListingID)

	_, _ = tx.ExecContext(ctx,
		`DELETE FROM Watchlist WHERE ListingID = ?`, offer.ListingID)

	txID, err := nextRecordID(ctx, "Transaction", "TransactionID")
	if err != nil {
		respondError(w, http.StatusInternalServerError, "transaction id allocation failed")
		return
	}
	res, err = tx.ExecContext(ctx,
		`INSERT INTO Transaction (TransactionID, ListingID, SellerID, BuyerID, OfferID, AgreedPrice)
         VALUES (?, ?, ?, ?, ?, ?)`,
		txID, offer.ListingID, offer.SellerID, offer.BuyerID, offerID, offer.OfferedPrice)
	if err != nil {
	...
	for _, o := range others {
		declTxID, _ := nextRecordID(ctx, "Transaction", "TransactionID")
		resDecl, errDecl := tx.ExecContext(ctx,
			`INSERT INTO Transaction (TransactionID, ListingID, SellerID, BuyerID, OfferID, AgreedPrice)
             VALUES (?, ?, ?, ?, ?, ?)`,
			declTxID, offer.ListingID, offer.SellerID, o.buyerID, o.id, o.price)
		if errDecl != nil {
			continue
		}
		_ = resDecl
		_, _ = tx.ExecContext(ctx,
			...
			o.buyerID, "Another buyer's offer was accepted on this listing. Your offer was declined.", offer.ListingID, o.id, declTxID)
	}

	_, _ = tx.ExecContext(ctx,
		`INSERT INTO Notification (RecipientID, NotificationType, Title, Message, RelatedListingID, RelatedOfferID, RelatedTransactionID)
         VALUES (?, 'OfferAccepted', 'Offer Accepted', 'Your offer has been accepted!', ?, ?, ?)`,
		offer.BuyerID, offer.ListingID, offerID, txID)

	_, _ = tx.ExecContext(ctx,
		`INSERT INTO Notification (RecipientID, NotificationType, Title, Message, RelatedListingID, RelatedOfferID, RelatedTransactionID)
         VALUES (?, 'TransactionCompleted', 'Sale recorded', 'You accepted an offer. A transaction was created for your listing.', ?, ?, ?)`,
		offer.SellerID, offer.ListingID, offerID, txID)
