...
func Register(w http.ResponseWriter, r *http.Request) {
	...
	// Atomic transaction for the central auth tables.
	...
	memberID, err := nextRecordID(r.Context(), "Member", "MemberID")
	if err != nil {
		respondError(w, http.StatusInternalServerError, "member id allocation failed")
		return
	}
	memberDB := memberShardDB(memberID)
	_, err = memberDB.ExecContext(r.Context(),
		`INSERT INTO Member (MemberID, user_id, Name, ContactNumber, Department, YearOfStudy, Hostel, RoomNumber, Bio)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		memberID, userID, req.Name, req.ContactNumber,
...
func Me(w http.ResponseWriter, r *http.Request) {
	...
	if mw.HasRole(ctx, "member") {
		...
		if memberID > 0 {
			...
			var yrStudy *int
			var isActive bool
			_ = memberShardDB(memberID).QueryRowContext(ctx,
