...
The Campus Trading application requires efficient data access over continuously growing datasets, including listings, members, offers, and transactions. Typical workloads include exact-key lookup (for example, by listing ID) and range-based filtering (for example, price windows). A linear baseline requires \(O(n)\) scans, which becomes a bottleneck as the dataset grows.

\subsection{Proposed Solution}
We implemented a B+ Tree-based indexing layer to provide stable and scalable DBMS-style performance. Under this design, point operations such as search, insert, update, and delete run in \(O(\log_m n)\), while range operations run in \(O(\log_m n + k)\). The linked-leaf structure supports ordered traversal directly, and balancing is preserved through split, borrow, and merge procedures during updates.
...
\midrule
\texttt{search(key)} & \(O(\log_m n)\) \\
\texttt{insert(key, value)} & \(O(\log_m n)\) \\
\texttt{delete(key)} & \(O(\log_m n)\) \\
\texttt{update(key, value)} & \(O(\log_m n)\) \\
\texttt{range\_query(start, end)} & \(O(\log_m n + k)\) \\
\texttt{get\_all()} & \(O(n)\) \\
\bottomrule
\end{tabular}
\end{table}
...
The observed trends are consistent with theoretical complexity and remain stable across tested scales. Search shows the clearest widening performance gap as \(n\) increases, matching \(O(\log_m n)\) behavior for B+ Tree versus \(O(n)\) behavior for brute-force search. Range query performance also improves substantially because the B+ Tree performs a single logarithmic seek and then scans linked leaves sequentially. Insert and delete remain scalable despite occasional split/merge overhead, and this overhead does not alter asymptotic behavior. Overall, B+ Tree consistently outperforms the brute-force baseline for larger workloads.
...
The Module A implementation demonstrates that B+ Tree indexing is a strong fit for Campus Trading workloads, particularly for mixed point-lookup and range-filter operations.
